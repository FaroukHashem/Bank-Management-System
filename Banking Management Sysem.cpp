...
{
	SoR = 3;
	if (client[send_clientnum].Amount == 0)// if amount in sending account =0
	{
	...
	trans:
		...
		cin >> transfer;
		if (!cin) //if the dont enter numbers.
		{
			cin.clear();
			cin.ignore(256, '\n');
			goto trans;
		}
		else if (transfer <= 0) //if the value of transfer equal 0 or -ve.
			goto trans;
		else if (client[send_clientnum].Amount < transfer * 1.015)//if client amount less than the transfer operation cost.
		{
...
void ATM::Withdrawal(int client_number) {
	double withdraw;
	if (client[client_number].Amount == 0)//if the user amount =0.
	{
	...
	draw:
		...
		cin >> withdraw;
		if (!cin)//if the user enter anything other than numbers.
		{
			cin.clear();
			cin.ignore(256, '\n');
			goto draw;
		}
		else if ((client[client_number].Amount - (withdraw * 1.03) < 0))//if client amount less than the withdrawal operation cost.
		{
...
label:
	...
	if (depos <= 0 || !cin)//if the user enter anything other than number.
	{
		cin.clear();
		cin.ignore(256, '\n');
		goto label;
...
int ATM::getcard() // check the card number is entered before or not && return the client number.
{
	int x = 0;
	int i;
	for (i = c; i >= 0; i--)
...
void ATM::RepairName(char name[]) {
	while (strlen(name) == 0)//ignoring pressing Enter immediately instead of entering a name.
	{
		Enter_Name();
	}
...
void ATM::Repaircard(char card[]) {
	...
	{
		cout << "The Card number must consists of 10 numbers,\n";
		Enter_cardnumber();
	}
